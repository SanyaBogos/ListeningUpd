...
namespace Listening.Web.Controllers.api.Custom
{
    public class AuthorizationController : BaseController
    {
        ...
        public AuthorizationController(
            ...
            ILogger<AuthorizationController> logger) : base(userManager)
        {
        ...
        public async Task<IActionResult> Exchange(OpenIdConnectRequest req)
        {
            ...
            if (req.IsPasswordGrantType())
            {
                ...
                if (user == null)
                    throw new ApiException("login_passwd_incorrect");

                //user.
                if (!user.IsEnabled)
                {
                    _logger.LogWarning(2, "Disabled user tries to login");
                    throw new ApiException("login_disabled_usr");
                }
        ...
        public async Task<IActionResult> Authorize(OpenIdConnectRequest req)
        {
            ...
            if (info == null)
            {
                ...
                var provider = (string)req["provider"];
