...
namespace Listening.Web.Extensions
{
    public static class ApplicationBuilderExtensions
    {
                   ...
                   .AddContentSecurityPolicy(builder =>
                    {
                        ...
                        var socketUrl = Startup.Configuration["HostUrl"].ToString()
        ...
        public static IApplicationBuilder AddDevMiddlewares(this IApplicationBuilder app)
        {
            ...
            var configSection = Startup.Configuration.GetSection("Logging");
            var logger = loggerFactory.CreateLogger("AppBuilderExtensions");
            logger.LogInformation(configSection.Value);
        ...
        public static IApplicationBuilder AddRestrictionsForSpec(this IApplicationBuilder app, string archievePath)
        {
            //var archieve = "/archive";

            app.UseStaticFiles(new StaticFileOptions()
            {
                OnPrepareResponse = s =>
                {
                    if (s.Context.Request.Path.StartsWithSegments(new PathString(archievePath)) &&
                       (!s.Context.User.Identity.IsAuthenticated || !(s.Context.User.IsInRole(GlobalConstats.SPECIFIC))))
                    {
                        s.Context.Response.StatusCode = StatusCodes.Status500InternalServerError;
